...
# Purpose     : Checks New CertMCapabilities values required on VTF sims
###########################################################################################################################
##########################################################################################################################
# Created by  : Mitali Sinha
# Created on  : 20.09.2017
# Purpose     : Checks Hw Inventory and PM file location on VTF sims
###########################################################################################################################
...
##############################TEST#########################
#Checking for VTF Nodes#

if  ($ARGV[2] !~m/^VTF/i)
{
print " Skipping this script as I am not a VTF Node.  \n";
exit;
}
...
$hwType=$hwType[1];
$serialNo_Value="D821781392";
$hwUnitLocation_Value="slot:0";
$hwType_Value="Card";
...
if ("$netsim_output[-1]" =~ "/rop") {
        print "PASSED: fileLocation is $netsim_output[-1]";
}
else {
        print "FAILED: fileLocation is not set to /rop";
}
...
#############################################
##############################################
# verify New CertMCapabilities values required
#############################################
@MMLCmds=(".open ".$SIMNAME,
            ".select ".$NODENAME,
            ".start ",
            "e X= csmo:ldn_to_mo_id(null,[\"ComTop:ManagedElement=$NODENAME\",\"ComTop:SystemFunctions=1\",\"RcsSecM:SecM=1\",\"RcsCertM:CertM=1\",\"RcsCertM:CertMCapabilities=1\"]).",
            "e csmo:get_attribute_value(null,X,enrollmentSupport)."
  );# end @MMLCmds
...
if ("$netsim_output[-1]" =~ "[0,1,3]") {
        print "PASSED: enrollmentSupport is $netsim_output[-1]";
}
else {
        print "FAILED: enrollmentSupport is not set to [0,1,3] ";
}
...
##############################
##############################################
# verify outputDirectory values required
#############################################
@MMLCmds=(".open ".$SIMNAME,
            ".select ".$NODENAME,
            ".start ",
            "e X= csmo:ldn_to_mo_id(null,[\"ComTop:ManagedElement=$NODENAME\",\"ComTop:SystemFunctions=1\",\"RcsPMEventM:PmEventM=1\",\"RcsPMEventM:EventProducer=VTFrat\",\"RcsPMEventM:FilePullCapabilities=2\"]).",
            "e csmo:get_attribute_value(null,X,outputDirectory)."
  );# end @MMLCmds
...
if ("$netsim_output[-1]" =~ "/pm_data") {
        print "PASSED: enrollmentSupport is $netsim_output[-1]";
}
else {
        print "FAILED: enrollmentSupport is not set to /pm_data ";
}
