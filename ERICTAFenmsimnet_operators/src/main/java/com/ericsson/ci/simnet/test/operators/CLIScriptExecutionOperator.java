...
public class CLIScriptExecutionOperator implements ScriptExecutionOperator {
    ...
    public int verifyScriptExecution(final String command) {
        ...
        String check5G = convertToPerlCmdArg(DataHandler.getAttribute("check5G").toString());
        String checkCORE = convertToPerlCmdArg(DataHandler.getAttribute("checkCORE").toString());
        String checkTransport = convertToPerlCmdArg(DataHandler.getAttribute("checkTransport").toString());
        String checkLTE = convertToPerlCmdArg(DataHandler.getAttribute("checkLTE").toString());
        String checkWRAN = convertToPerlCmdArg(DataHandler.getAttribute("checkWRAN").toString());
        String checkGRAN = convertToPerlCmdArg(DataHandler.getAttribute("checkGRAN").toString());
        String FirstServer = convertToPerlCmdArg(DataHandler.getAttribute("FirstServer").toString());
        ...
        try {
            ...
            if (command.contains("simnet") && command.contains("initiateCheck.pl")) {
            	simnetParam = "-overwrite " + "-check5G " + check5G + " -checkCORE " + checkCORE + " -checkTransport " 
            + checkTransport + " -checkLTE " + checkLTE + " -checkWRAN " + checkWRAN + " -checkGRAN " + checkGRAN;
             ...
             else if (command.contains("simnet") && command.contains("nssSimulationdefinitioncheck_LRAN.sh")) {
                     logger.info("Command to be executed: {}", command.concat(FirstServer));
                     cliCmdHelper.execute(command.concat(FirstServer));
            }
			else if (command.contains("simnet") && command.contains("nssSimulationdefinitioncheck_GRAN.sh")) {
			         simnetParams = FirstServer + " " + NRM;
                     logger.info("Command to be executed: {}", command.concat(FirstServer));
                     cliCmdHelper.execute(command.concat(FirstServer));
            }
			else if (command.contains("simnet") && command.contains("nssSimulationdefinitioncheck_WRAN.sh")) {
                     logger.info("Command to be executed: {}", command.concat(FirstServer));
                     cliCmdHelper.execute(command.concat(FirstServer));
            }
			
			else if (command.contains("simnet") && command.contains("nssSimulationdefinitioncheck_NONRAN.sh")) {
			         simnetParams = FirstServer + " " + NRM;
                     logger.info("Command to be executed: {}", command.concat(simnetParams));
                     cliCmdHelper.execute(command.concat(simnetParams));
            }
          else if (command.contains("simnet") && command.contains("nssSimulationdefinitioncheck_5g.sh")) {
			         simnetParams = FirstServer + " " + NRM;
                     logger.info("Command to be executed: {}", command.concat(simnetParams));
                     cliCmdHelper.execute(command.concat(simnetParams));
            }
			
		   else if (command.contains("simnet") && command.contains("CheckProdData.sh")) {
			         //simnetParams = FirstServer + " " + NRM;
                     //logger.info("Command to be executed: {}", command.concat(simnetParams));
                     //cliCmdHelper.execute(command.concat(simnetParams));
